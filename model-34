...
from PyQt5.QtWidgets import QApplication, QPushButton, QGraphicsDropShadowEffect, QVBoxLayout, QWidget
from PyQt5.QtGui import QIcon, QColor, QFont
from PyQt5.QtCore import QSize, QPropertyAnimation, QPoint, QEasingCurve
...
from PyQt5.QtWidgets import QDialog, QVBoxLayout, QGroupBox, QFormLayout, QLabel, QSlider, QPushButton, QFrame, QMessageBox, QStyleFactory
...
class SettingsWindow(QDialog):
    ...
    def init_ui(self):
        ...
        # Stylesheet for the whole window
        self.setStyleSheet("""
            QLabel {
                color: #ffffff;
            }
            QPushButton {
                color: #ffffff;
                background-color: #333333; /* Optional: Set background color for buttons */
                border: 1px solid #555555; /* Optional: Set border color for buttons */
            }
        ...
        general_group2.setLayout(general_layout)
        main_layout.addWidget(general_group2)
    ...
    def save_settings(self):
        ...
        print(f"Gespeicherte Helligkeit: {brightness}")
        print(f"Gespeicherte Lautstärke: {volume}")
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.button1.setIconSize(QtCore.QSize(70, 70))  # Hier Größe der SVG-Datei angeben
        self.button1.setIconSize(QSize(70, 70))  # Hier Größe der SVG-Datei angeben
        ...
        self.button2.setIconSize(QSize(70, 70))  # Hier Größe der SVG-Datei angeben
        ...
        self.shadow_effect.setColor(QColor(0, 0, 0, 160))  # Schwarze Farbe für den Schatten
        self.shadow_effect.setOffset(7, 7)
        self.button2.setGraphicsEffect(self.shadow_effect)

        self.button_layout.addWidget(self.button2)

        # Animationen erstellen
        self.animation_up = QPropertyAnimation(self.button2, b"pos")
        self.animation_up.setDuration(200)
        self.animation_up.setEasingCurve(QEasingCurve.OutQuad)
        self.animation_up.setEndValue(QPoint(self.button2.x(), self.button2.y() - 5))

        self.animation_down = QPropertyAnimation(self.button2, b"pos")
        self.animation_down.setDuration(200)
        self.animation_down.setEasingCurve(QEasingCurve.OutQuad)
        self.animation_down.setEndValue(QPoint(self.button2.x(), self.button2.y() + 5))

        self.shadow_animation_rotate_up = QPropertyAnimation(self.shadow_effect, b"offset")
        self.shadow_animation_rotate_up.setDuration(200)
        self.shadow_animation_rotate_up.setEasingCurve(QEasingCurve.OutQuad)
        self.shadow_animation_rotate_up.setEndValue(QPoint(10, 10))  # Leicht gedreht

        self.shadow_animation_rotate_down = QPropertyAnimation(self.shadow_effect, b"offset")
        self.shadow_animation_rotate_down.setDuration(200)
        self.shadow_animation_rotate_down.setEasingCurve(QEasingCurve.OutQuad)
        self.shadow_animation_rotate_down.setEndValue(QPoint(7, 7))  # Zurück zur Originalposition

        # Button-Events verbinden
        self.button2.pressed.connect(self.animate_press)
        self.button2.released.connect(self.animate_release)
        ...
        shadow_effect.setColor(QColor(0, 0, 0, 160))  # Schwarze Farbe für den Schatten
        shadow_effect.setOffset(7, 7)
        self.button4.setIconSize(QtCore.QSize(60, 60))  # Hier Größe der SVG-Datei angeben
        self.button4.setIconSize(QSize(60, 60))  # Hier Größe der SVG-Datei angeben
        self.button4.setGraphicsEffect(shadow_effect)
        ...
        # Definiere eine Funktion, um den Button um 10px nach unten zu verschieben.
        def move_button4_down(button):
            if not button.isChecked():
                button.move(button.x(), button.y() + 5)

        # Verbinde die Funktionen mit den entsprechenden Signalen.

        self.button4.pressed.connect(move_button4_up)
        self.button4.released.connect(lambda: move_button4_down(self.button4))

        # Verbinde die enterEvent und leaveEvent mit den Funktionen.
        self.button4.enterEvent = lambda event: move_button4_up()
        self.button4.leaveEvent = lambda event: move_button4_down(self.button4)
    ...
    def animate_press(self):
        ...
        self.shadow_animation_rotate_up.start()

    def animate_release(self):
        self.animation_down.start()
        self.shadow_animation_rotate_down.start()

    def eventFilter(self, obj, event):
        if obj == self.button2 and event.type() == event.Enter:
            self.animate_press()
        elif obj == self.button2 and event.type() == event.Leave:
