...
from PyQt5.QtGui import QPixmap, QImageReader, QPalette, QColor, QPainter, QBrush
from PyQt5.QtCore import Qt, QSize

class GlassEffectWidget(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setAttribute(Qt.WA_TranslucentBackground)  # Translucent background for glass effect
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint)
        self.setStyleSheet("background: rgba(0, 0, 0, 100);")  # Semi-transparent background

    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setCompositionMode(QPainter.CompositionMode_Source)
        painter.fillRect(self.rect(), QBrush(QColor(0, 0, 0, 100)))  # Apply a glass-like effect
        painter.end()

class ImageGallery(GlassEffectWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def init_ui(self):
        ...
        layout = QHBoxLayout(self)  # Horizontal layout for horizontal scrolling
        layout.setContentsMargins(0, 0, 0, 0)

        # Setze die Größe des Fensters auf 400x400 Pixel
        self.setGeometry(0, 300, 1728, 300)

        # ScrollArea zur Anzeige der Bilder
        scroll_area = QScrollArea(self)
        scroll_area.setWidgetResizable(True)
        scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOn)  # Always show horizontal scrollbar
        scroll_area.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # Disable vertical scrollbar

        # Erstelle ein Widget zum Halten der Bilder im ScrollArea
        scroll_content = QWidget()
        self.scroll_layout = QHBoxLayout(scroll_content)  # Use horizontal layout for horizontal scrolling
        ...
        # Setze Layout für das Haupt-Widget
        ...
        try:
            for img_name in os.listdir(self.image_folder):
                if any(img_name.lower().endswith(ext) for ext in supported_extensions):
                    ...
                    # Skaliere das Bild, um es in die Größe von 400x300 anzupassen
                    ...
                    # Füge das Label zum ScrollLayout hinzu
...
class MainWindow(QMainWindow):
    ...
    def on_button5_leave(self, event):
        self.shadow_effect5.setOffset(7, 7)
        self.move_button5_down(self.button4)
        event.accept()
    ...
    def show_gallery(self):
        ...
        # Remove existing ImageGallery if it exists
        ...
        self.image_gallery.set_image_folder(image_folder)  # Set the image folder after instantiation
