...
class CameraThread(QThread):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.running = True
        self.fps = 30  # Ziel-Framerate (kann auf 60 geändert werden, wenn die Kamera dies unterstützt)
    ...
    def run(self):
        try:
            ...
            cap.set(cv2.CAP_PROP_EXPOSURE, -4)  # Belichtung (Wert anpassen)
            cap.set(cv2.CAP_PROP_AUTO_WB, 1)    # Weißabgleich auf automatisch setzen

            while self.running:
                ret, frame = cap.read()
                if ret:
                    # Optional: Bildverarbeitung hinzufügen
    ...
    def process_image(self, frame):
        ...
        return frame  # Keine zusätzliche Verarbeitung
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setGeometry(96, 52, 1728, 972)
