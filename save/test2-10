...
class VideoPlayer(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.text_label2.setStyleSheet("font-size: 50px; color: white; font-family: 'Courier New';")
    ...
    def update_text2(self):
        try:
            logging.debug(f"Updating text2: {self.current_text_index2}/{len(self.full_textt)}")
            if self.current_text_index2 < len(self.full_textt):
                self.text_label2.setText(self.full_textt[:self.current_text_index2 + 1])
            ...
            logging.error(f"Error in update_text2: {e}")
    ...
    def remove_text2(self):
        ...
        if self.current_text_index2 > 0:
            self.current_text_index2 -= 1
            self.text_label2.setText(self.full_textt[:self.current_text_index2])
        else:
