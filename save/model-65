...
class CameraThread(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.corner_length = 20  # Länge der Ecken
...
import os
import ctypes
from PyQt5.QtWidgets import QWidget, QHBoxLayout, QLabel, QScrollArea, QVBoxLayout, QPushButton, QFrame
...
class ImageGallery(QWidget):
    def __init__(self, parent=None):
        ...
        # Initialisiere den Bildordner
        self.image_folder = r"C:\Users\julia\PycharmProjects\Xcpp\img"  # Standardordner
        self.init_ui()

    def init_ui(self):
        # Hauptlayout für die Galerie
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)

        # Setze die Größe des Fensters auf 1728x300 Pixel
        self.setGeometry(0, 475, 1728, 300)
        ...
        # Setze den Glas-Hintergrund-Effekt
        self.setAttribute(Qt.WA_TranslucentBackground)
        self.setStyleSheet("background: rgba(255, 255, 255, 0);")  # Transparentes Fenster

        # Erstelle den Glas-Effekt-Rahmen
        glass_frame = QFrame(self)
        glass_frame.setGeometry(0, 0, 1728, 300)
        glass_frame.setStyleSheet("""
            background-color: rgba(255, 255, 255, 0.5);  # Transparenter Hintergrund
            color: #000000;
            border-radius: 10px;
        """)

        # Erstelle und konfiguriere den Schließen-Button
        self.close_button = QPushButton('X', self)
        self.close_button.setGeometry(self.width() - 40, 0, 40, 30)  # Positioniere den Button in der oberen rechten Ecke
        self.close_button.setStyleSheet("background-color: red; color: white; font-weight: bold; border: none;")
        self.close_button.clicked.connect(self.close)

        # ScrollArea zur Anzeige der Bilder
        scroll_area = QScrollArea(self)
        scroll_area.setWidgetResizable(True)
        scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # Keine horizontale Scrollbar
        scroll_area.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # Keine vertikale Scrollbar

        # Erstelle ein Widget zum Halten der Bilder im ScrollArea
        self.scroll_content = QWidget()
        self.scroll_layout = QHBoxLayout(self.scroll_content)
        self.scroll_layout.setContentsMargins(0, 0, 0, 0)
        self.scroll_layout.setSpacing(0)

        # Setze das Content-Widget im ScrollArea
        scroll_area.setWidget(self.scroll_content)

        # Füge den ScrollArea zum Hauptlayout hinzu
    ...
    def load_images(self):
        ...
        # Lösche vorhandene Widgets im Layout
        ...
        try:
            for img_name in os.listdir(self.image_folder):
                if any(img_name.lower().endswith(ext) for ext in supported_extensions):
                    ...
                    # Skaliere das Bild auf 400x300
                    pixmap = pixmap.scaled(400, 300, aspectRatioMode=Qt.KeepAspectRatio,
                                           transformMode=Qt.SmoothTransformation)

                    # Erstelle ein Label und setze das skalierte Pixmap
                    label = QLabel(self)
                    label.setPixmap(pixmap)
                    label.setStyleSheet("background: rgba(0, 0, 0, 0);")  # Transparentes Label-Hintergrund

                    # Füge das Label zum Layout hinzu
    ...
    def paintEvent(self, event):
        ...
        # Erstelle einen Farbverlauf für den Fade-Out-Effekt
        gradient = QLinearGradient(self.width() - 100, 0, self.width(), 0)
        gradient.setColorAt(0, QColor(0, 0, 0, 0))  # Transparent
        gradient.setColorAt(1, QColor(0, 0, 0, 100))  # Schwarz mit Transparenz

        painter.setBrush(QBrush(gradient))
        painter.setPen(Qt.NoPen)
        painter.drawRect(QRect(self.width() - 100, 0, 100, self.height()))  # Draw gradient rectangle

    def resizeEvent(self, event):
        super().resizeEvent(event)
        # Aktualisiere die Position des Schließen-Buttons bei einer Größenänderung des Fensters
        self.close_button.setGeometry(self.width() - 40, 0, 40, 30)
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.update_button_icon()  # Setze das neueste Bild als Icon
        ...
        self.button5.setStyleSheet("""
            QPushButton {
                ...
                border: 2px solid #000000;  # Optional: schwarzer Rand für bessere Sichtbarkeit
            }
        """)
    ...
    def get_latest_image(self):
        ...
        # Finde die neueste Datei im Verzeichnis
        latest_file = max(
    ...
    def update_button_icon(self):
        ...
        if latest_image:
            # Setze das neueste Bild als Icon für den Button
            self.button5.setIcon(QIcon(latest_image))
        else:
            # Setze ein Standard-Icon, falls keine Bilder vorhanden sind
            self.button5.setIcon(QIcon())
    ...
    def capture_image(self):
        try:
            ...
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 672)
