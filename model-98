...
class SettingsWindow(QDialog):
    ...
    def init_ui(self):
        ...
        general_layout.addRow(QLabel(f""))
        general_layout.addRow(QLabel(f"Xcpp 13B uses <span style='color: blue;'>13 GB</span> of your RAM"))
        general_layout.addRow(QLabel(f"Xcpp 13B uses <span style='color: blue;'>91%</span> of your CPU power"))
        general_layout.addRow(QLabel(f"Xcpp 13B uses <span style='color: blue;'>79 GB</span> of your disk space"))
...
class ModelWindow(QDialog):
    ...
    def init_ui(self):
        ...
        # Füge die Informationen in das Layout ein
        general_layout.addRow(QLabel(f""))
        general_layout.addRow(QLabel(f"Thank you for choosing Xc++.\nXc++ represents a significant advancement in natural human-computer interaction. It accepts a wide range of inputs, including text, audio, and images, and can generate outputs in both text and audio formats. Xc++ excels particularly in vision and audio comprehension, offering superior performance compared to existing models."))
        general_layout.addRow(QLabel(f"RAM: <span style='color: blue;'>{ram:.2f} GB</span>"))
        general_layout.addRow(QLabel(f"Number of Threads: <span style='color: blue;'>{cpu_threads}</span>"))
        general_layout.addRow(QLabel(f"Chip type/CPU model: <span style='color: blue;'>{cpu_model}</span>"))
        general_layout.addRow(QLabel(f"Graphics card(s): <span style='color: blue;'>{gpu}</span>"))
